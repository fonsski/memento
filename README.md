...
Кроме встроенной темы («Ink & Patina»), Memento можно оформить своей
темой, описанной JSON-файлом. Иконка палитры в боковой панели открывает
диалог выбора темы — там же показан точный путь до папки с темами на
вашей машине (путь зависит от ОС, поэтому он показывается в самом
приложении, а не зафиксирован здесь).

Формат файла — `<любое_имя>.json` в этой папке:
...
```json
{
  ...
  "dark": {
    "accent": "#2E86AB",
    "backgroundPrimary": "#0A0F14"
  },
  "light": {
    "accent": "#1B4F72",
    "backgroundPrimary": "#F3F8FA"
  }
}
```

- `name` — отображаемое название темы в диалоге выбора.
- `dark` / `light` — независимые наборы цветов для тёмного и светлого
  режима; можно указать только часть полей — остальные возьмутся из
  встроенной темы для этого режима. Значения — hex-цвета `#RRGGBB` или
  `#AARRGGBB` (с альфа-каналом).

Доступные поля цвета внутри `dark`/`light`: `backgroundPrimary`,
`backgroundSecondary`, `navPanel`, `card`, `divider`, `textPrimary`,
`textSecondary`, `textDisabled`, `link`, `accent`, `onAccent`, `error`,
`warning`, `success`, `info`. Цвета для наведения/выделения/подсветки
поиска отдельно не настраиваются — они автоматически считаются из
`accent` (и от того, тёмная тема или светлая), чтобы акцентный цвет
темы последовательно управлял всеми интерактивными подсветками, а не
только кнопками.

Полный рабочий пример — [`docs/example-theme.json`](docs/example-theme.json).
